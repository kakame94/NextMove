...
| Apple Watch | `ios/KlarisWatch/{KlarisWatchApp.swift,WatchSession.swift}` + `ios/Runner/WatchBridge.swift` + `data/services/watch_bridge_service.dart` | watchOS 10+ companion, KPI cards (chauds + RDV), liste agenda jour, dictation memo, WatchConnectivity bidirectionnel |
...
| Localization es-MX | enum `KlarisLang.es` + `_es` override map dans klaris_strings.dart + segmented FR/EN/ES | Fallback chain ES → EN → key, 35 traductions Spanish prioritaires |
